...
describe("parseWebhookBody", () => {
  ...
  it("returns the event if the event signatures match", () => {
    expect(parseWebhookBody(client, secret, signature, body)).toEqual(
      JSON.parse(body)
    );
  });
  ...
  it("returns null if the event signatures doesn't match", () => {
    ...
    expect(parseWebhookBody(client, "nope", signature, body)).toBe(null);
    expect(parseWebhookBody(client, secret, "nope", body)).toBe(null);
  });
});
